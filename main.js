...
gameButton.addEventListener('click', (event) => {
    if(event.target.className === 'ui__game-button' ||
    event.target.parentNode.className === 'ui__game-button') {
        time = setInterval(secondTimer, 1000);
        targetStart();
        countCarrot();
    } else if(event.target.className === 'ui__game-button reset' ||
    event.target.parentNode.className === 'ui__game-button reset') {
...
target.addEventListener('click', (event) => {
    if(event.target.className === 'game-carrot active') {
        ...
        clearInterval(time);
        gameButton.classList.add('reset');
    }
})
...
function targetRespon() {
    ...
    cFind = 0;
    currentSecond = 10;
    gameButton.classList.remove('reset');
}